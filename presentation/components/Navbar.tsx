...
// Fix: added Badge to imports from atoms
    ...
    <div className="container mx-auto px-4 h-14 flex items-center justify-between">
      <div className="flex items-center gap-8">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm font-medium">
          <button 
            onClick={() => setView('EXPLORE')} 
            className={currentView === 'EXPLORE' ? 'text-slate-900' : 'text-slate-500 hover:text-slate-900'}
          >
            Explorar
          </button>
          <button 
            onClick={() => setView('DASHBOARD')} 
            className={currentView === 'DASHBOARD' ? 'text-slate-900' : 'text-slate-500 hover:text-slate-900'}
          >
            Dashboard
          </button>
        </nav>
