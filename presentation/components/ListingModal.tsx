...
export const ListingModal: React.FC<Props> = ({ onClose, onSubmit, aiService }) => {
  ...
  const handleAi = async () => {
    ...
    setLoading(true);
    const res = await aiService.getCarContent(data.brand, data.model, data.year);
    setData({ ...data, description: res.description, price: res.suggestedPrice });
    setLoading(false);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
      <Card className="w-full max-w-lg shadow-2xl relative">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          ...
          {/* Fix: removed unsupported size="sm" prop */}
